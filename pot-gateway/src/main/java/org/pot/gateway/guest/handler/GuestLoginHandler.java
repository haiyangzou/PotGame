...
public final class GuestLoginHandler extends GuestRequestHandler<LoginReqC2S> {
    ...
    protected boolean handleRequest(Guest guest, LoginReqC2S request) throws Exception {
        try {
            ...
            if (!success) {
                log.error("Guest login add remote user fail. account={},conn={},request={}", request.getAccount(), guest.getConnection(), ProtocolPrinter.printJson(request));
                throw new ServiceException(CommonErrorCode.LOGIN_FAIL);
            }
        } catch (Throwable throwable) {
            loginFail(guest, CommonErrorCode.LOGIN_FAIL.getErrorCode());
            throw throwable;
        }
        return false;
    }
