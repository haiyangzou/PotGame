...
public class TimeUtil {
   ...
   * yyyy-MM-dd HH:mm:ss
   */
  public final static DateTimeFormatter YYYYMMDDHHMMSS = DateTimeFormatter
      .ofPattern("yyyy-MM-dd HH:mm:ss").withLocale(Locale.getDefault());
  /**
   * yyyy.MM.dd-HH:mm
   */
  public final static DateTimeFormatter YYYYMMDDGHHMM = DateTimeFormatter
      .ofPattern("yyyy.MM.dd-HH:mm").withLocale(Locale.getDefault());
  /**
   * yyyy-MM-dd HH:mm
   */
  public final static DateTimeFormatter YYYYMMDDHHMM = DateTimeFormatter
  ...
  public static long getOffToDayZeroMil(int days) {
    try {
      ZonedDateTime ldt = ZonedDateTime
          .ofInstant(Instant.ofEpochMilli(currentTimeMillis()), ZoneId.systemDefault());
      if (days < 0) {
        ldt = ldt.minusDays(-days);
      } else {
  ...
  public static int getMinDaysOfMonth() {
    return LocalDateTime
        .ofInstant(Instant.ofEpochMilli(currentTimeMillis()), ZoneId.systemDefault()).getMonth()
        .minLength();
  }
  ...
  public static int getHour() {
    return LocalDateTime
        .ofInstant(Instant.ofEpochMilli(currentTimeMillis()), ZoneId.systemDefault()).getHour();
  }
  ...
  public static int getSecond() {
    return LocalDateTime
        .ofInstant(Instant.ofEpochMilli(currentTimeMillis()), ZoneId.systemDefault()).getSecond();
  }
  ...
  public static int getMinute() {
    return LocalDateTime
        .ofInstant(Instant.ofEpochMilli(currentTimeMillis()), ZoneId.systemDefault()).getMinute();
  }
  ...
  public static int getYear() {
    return LocalDateTime
        .ofInstant(Instant.ofEpochMilli(currentTimeMillis()), ZoneId.systemDefault()).getYear();
  }
  ...
  public static int getDayOfWeek() {
    return LocalDateTime
        .ofInstant(Instant.ofEpochMilli(currentTimeMillis()), ZoneId.systemDefault()).getDayOfWeek()
        .getValue();
  }
  ...
  public static String getNowWeekMondayFormat(DateTimeFormatter formatter) {
    LocalDateTime ldt = LocalDateTime
        .ofInstant(Instant.ofEpochMilli(currentTimeMillis()), ZoneId.systemDefault());
    ldt = LocalDateTime
        .of(ldt.minusDays(ldt.getDayOfWeek().getValue() - 1).toLocalDate(), LocalTime.MIN);
    return ldt.format(formatter);
  }
  ...
  public static boolean isSameWeek(long time1, long time2) {
    LocalDateTime ldt1 = LocalDateTime
        .ofInstant(Instant.ofEpochMilli(time1), ZoneId.systemDefault());
    LocalDateTime ldt2 = LocalDateTime
        .ofInstant(Instant.ofEpochMilli(time2), ZoneId.systemDefault());
    TemporalField woy = WeekFields.of(Locale.getDefault()).weekOfWeekBasedYear();
    return ldt1.getYear() == ldt2.getYear() && ldt1.get(woy) == ldt2.get(woy);
  }

  /**
   * 判断两个时间是否在同一周(注意这里周日和周一不是在一周里的)
   *
   * @param time1
   * @param time2
   * @return
   */
  public static boolean isSameWeekTwo(long time1, long time2) {
  ...
  public static boolean isSameMinu(long time1, long time2) {
    return LocalDateTime.ofInstant(Instant.ofEpochMilli(time1), ZoneId.systemDefault()).getMinute()
        == LocalDateTime.ofInstant(Instant.ofEpochMilli(time2), ZoneId.systemDefault()).getMinute();
  }
...
//		return Clock.systemDefaultZone().instant().getEpochSecond() + timeOffset / 1000;
  ...
  public static long getTimeByLocal(int year, int month, int day, int hour, int minte, int second) {
    return LocalDateTime.of(year, month, month, hour, minte, second).atZone(ZONE_OFFSET).toInstant()
        .toEpochMilli();
  }
  ...
  public static Long getTodayZeroPointTimestamps() {
    Long currentTimestamps = currentTimeMillis();
